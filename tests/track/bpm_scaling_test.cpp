...
TEST(BpmScalingTest, HighBpmIncreasesAverageNoteDuration) {
  ...
  EXPECT_GE(avg_170, avg_120 * 0.8)
      << "BPM 170 average duration (" << avg_170 << ") should be at least 80% of BPM 120 average ("
      << avg_120 << ") due to long_note_boost scaling";
}
...
TEST(BpmScalingTest, VocalExtensionNoSustainedDissonance) {
  ...
  for (const auto& voc : vocal_notes) {
    ...
    for (const auto& mot : motif_notes) {
      ...
      int interval = std::abs(static_cast<int>(voc.note) - static_cast<int>(mot.note));
  ...
  double clash_ratio = static_cast<double>(severe_clash_count) / static_cast<double>(total_notes);
  EXPECT_LT(clash_ratio, 0.05) << "Severe vocal-motif clashes (m2/m9 with overlap >= 1 beat) "
...
TEST(BpmScalingTest, BreathDurationMinimum150ms) {
  ...
  Tick expected_min =
      static_cast<Tick>(kMinBreathSeconds * kHighBpm * TICKS_PER_BEAT / 60.0f);  // 204
  ...
  for (auto section : sections) {
    Tick breath =
        melody::getBreathDuration(section, Mood::AnimeHighEnergy,
                                  /* phrase_density */ 2.0f,
                                  /* phrase_high_pitch */ 72,
                                  /* ctx */ nullptr, VocalStylePreset::Standard, kHighBpm);
    EXPECT_GE(breath, expected_min)
        << "Breath duration at BPM " << kHighBpm << " for section " << static_cast<int>(section)
...
TEST(BpmScalingTest, BreathDurationScalesWithBpm) {
  ...
  Tick breath_120 = melody::getBreathDuration(SectionType::A, Mood::StraightPop, 1.0f, 65, nullptr,
                                              VocalStylePreset::Standard, 120);

  Tick breath_170 = melody::getBreathDuration(SectionType::A, Mood::StraightPop, 1.0f, 65, nullptr,
                                              VocalStylePreset::Standard, 170);

  Tick expected_120 = static_cast<Tick>(kMinBreathSeconds * 120 * TICKS_PER_BEAT / 60.0f);  // 144
  Tick expected_170 = static_cast<Tick>(kMinBreathSeconds * 170 * TICKS_PER_BEAT / 60.0f);  // 204

  EXPECT_GE(breath_120, expected_120) << "BPM 120: breath should be >= 144 ticks";
  EXPECT_GE(breath_170, expected_170) << "BPM 170: breath should be >= 204 ticks";
