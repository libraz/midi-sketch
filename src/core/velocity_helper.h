...
inline uint8_t clamp(int raw) { return static_cast<uint8_t>(std::clamp(raw, 1, 127)); }

/// @brief Clamp a float velocity value to valid MIDI range [1, 127].
/// @param raw Raw velocity value (may be out of range)
/// @return Clamped uint8_t velocity
inline uint8_t clamp(float raw) { return static_cast<uint8_t>(std::clamp(raw, 1.0f, 127.0f)); }
...
inline uint8_t scale(uint8_t base, float ratio) {
  return static_cast<uint8_t>(
      std::clamp(static_cast<int>(static_cast<float>(base) * ratio), 1, 127));
}
