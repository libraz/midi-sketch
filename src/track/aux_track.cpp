...
MidiTrack AuxTrackGenerator::generate(const AuxConfig& config, const AuxContext& ctx,
  ...
  for (const auto& note : notes) {
    track.addNote(
        factory.create(note.start_tick, note.duration, note.note, note.velocity, NoteSource::Aux));
  }
...
void AuxTrackGenerator::generateFullTrack(MidiTrack& track, const SongContext& song_ctx,
  ...
  for (size_t i = 0; i < notes.size(); ++i) {
    ...
    if (chord_change > 0 && chord_change > note.start_tick && chord_change < note_end) {
      ...
      if (!is_chord_tone) {
        ...
        for (int tone : new_chord_tones) {
          for (int oct_offset = -1; oct_offset <= 1; ++oct_offset) {
            ...
            if (dist < best_dist && dist > 0) {
              ...
              bool clashes = !harmony.isPitchSafe(static_cast<uint8_t>(candidate), chord_change,
                                                  time_after_change, TrackRole::Aux);
              if (!clashes) {
                best_dist = dist;
        ...
        if (time_before_change < kAnticipationThreshold) {
        ...
        } else if (time_before_change >= kMinNoteDuration &&
                   time_after_change >= kMinNoteDuration) {
          ...
          NoteFactory factory(harmony);
          auto safe_note = factory.createIfNoDissonance(chord_change, time_after_change,
...
std::vector<NoteEvent> AuxTrackGenerator::generatePulseLoop(const AuxContext& ctx,
  ...
  while (current_tick < ctx.section_end) {
    ...
    pitch = getBestAvailablePitch(pitch, current_tick, note_duration, ctx.main_melody, harmony, aux_low,
  ...
  if (ctx.rest_positions != nullptr && !ctx.rest_positions->empty()) {
    ...
    for (const Tick& rest_start : *ctx.rest_positions) {
      ...
      // Check for safety
      response_pitch = getBestAvailablePitch(response_pitch, rest_start, TICK_QUARTER, ctx.main_melody,
...
std::vector<NoteEvent> AuxTrackGenerator::generateTargetHint(const AuxContext& ctx,
                                                             const AuxConfig& config,
                                                             const IHarmonyContext& harmony,
                                                             std::mt19937& rng) {
  std::vector<NoteEvent> result;
  ...
  for (Tick phrase_end : phrase_ends) {
    ...
    pitch = getBestAvailablePitch(pitch, hint_start, TICK_QUARTER, ctx.main_melody, harmony, aux_low,
                         aux_high, ctx.chord_degree, meta.dissonance_tolerance);
...
std::vector<NoteEvent> AuxTrackGenerator::generateGrooveAccent(const AuxContext& ctx,
                                                               const AuxConfig& config,
                                                               const IHarmonyContext& harmony,
                                                               std::mt19937& rng) {
  std::vector<NoteEvent> result;
  ...
  while (current_bar < ctx.section_end) {
    ...
    if (beat2 >= ctx.section_start && beat2 < ctx.section_end) {
      ...
      if (density_dist(rng) < config.density_ratio * meta.base_density) {
        ...
        uint8_t pitch =
            getBestAvailablePitch(root_pitch, beat2, TICK_EIGHTH, ctx.main_melody, harmony, aux_low,
                         aux_high, ctx.chord_degree, meta.dissonance_tolerance);
    ...
    if (beat4 >= ctx.section_start && beat4 < ctx.section_end) {
      ...
      if (density_dist(rng) < config.density_ratio * meta.base_density) {
        uint8_t pitch =
            getBestAvailablePitch(root_pitch, beat4, TICK_EIGHTH, ctx.main_melody, harmony, aux_low,
                         aux_high, ctx.chord_degree, meta.dissonance_tolerance);
  ...
  if (ctx.rest_positions != nullptr && !ctx.rest_positions->empty()) {
    ...
    for (const Tick& rest_start : *ctx.rest_positions) {
      ...
      // Check for safety
      accent_pitch = getBestAvailablePitch(accent_pitch, rest_start, TICK_EIGHTH, ctx.main_melody,
                                  harmony, aux_low, aux_high, rest_chord_degree,
                                  meta.dissonance_tolerance);
...
std::vector<NoteEvent> AuxTrackGenerator::generatePhraseTail(const AuxContext& ctx,
                                                             const AuxConfig& config,
                                                             const IHarmonyContext& harmony,
                                                             std::mt19937& rng) {
  std::vector<NoteEvent> result;
  ...
  for (const auto& [phrase_end, last_pitch] : phrase_info) {
    ...
    uint8_t pitch =
        getBestAvailablePitch(static_cast<uint8_t>(tail_pitch), tail_start, TICK_EIGHTH, ctx.main_melody,
...
std::vector<NoteEvent> AuxTrackGenerator::generateEmotionalPad(const AuxContext& ctx,
                                                               const AuxConfig& config,
                                                               const IHarmonyContext& harmony,
                                                               std::mt19937& rng) {
  std::vector<NoteEvent> result;
  ...
  while (current_tick < ctx.section_end) {
    ...
    // Root note (always)
    uint8_t safe_root =
        getBestAvailablePitch(root_pitch, current_tick, actual_duration, ctx.main_melody, harmony, aux_low,
    ...
    if (voice_count >= 2 &&
      ...
      uint8_t safe_fifth =
          getBestAvailablePitch(fifth_pitch, current_tick, actual_duration, ctx.main_melody, harmony,
    ...
    if (is_section_ending && voice_count >= 2) {
      ...
      if (tension_dist(rng) < 0.5f) {  // 50% chance of tension
        ...
        uint8_t safe_tension =
            getBestAvailablePitch(tension_pitch, current_tick, actual_duration, ctx.main_melody, harmony,
...
void AuxTrackGenerator::calculateAuxRange(const AuxConfig& config,
...
bool AuxTrackGenerator::isPitchSafe(uint8_t pitch, Tick start, Tick duration,
...
uint8_t AuxTrackGenerator::getBestAvailablePitch(uint8_t desired, Tick start, Tick duration,
                                        const std::vector<NoteEvent>* main_melody,
...
std::vector<NoteEvent> AuxTrackGenerator::generateHarmony(const AuxContext& ctx,
                                                          const AuxConfig& config,
                                                          const IHarmonyContext& harmony,
...
std::vector<NoteEvent> AuxTrackGenerator::generateMelodicHook(const AuxContext& ctx,
                                                              const AuxConfig& config,
                                                              const IHarmonyContext& harmony,
                                                              std::mt19937& rng) {
  std::vector<NoteEvent> result;
  ...
  for (int i = 0; i < NOTES_PER_BAR * 2; ++i) {
    ...
    pitch = getBestAvailablePitch(static_cast<uint8_t>(pitch), current_tick, NOTE_DURATION, ctx.main_melody,
  ...
  for (int phrase = 0; phrase < phrases_needed; ++phrase) {
    ...
    for (const auto& note : base_hook) {
      ...
      hook_note.note =
          getBestAvailablePitch(hook_note.note, hook_note.start_tick, hook_note.duration, ctx.main_melody,
...
std::vector<NoteEvent> AuxTrackGenerator::generateMotifCounter(const AuxContext& ctx,
                                                               const AuxConfig& config,
                                                               const IHarmonyContext& harmony,
  ...
  for (const auto& phrase : vocal_analysis.phrases) {
    ...
    while (current_tick < phrase_end) {
      ...
      if (next_chord_change > 0 && next_chord_change > current_tick &&
        ...
        counter_pitch =
            std::clamp(counter_pitch, static_cast<int>(aux_low), static_cast<int>(aux_high));
        current_chord_degree = next_chord_degree;  // Update for getBestAvailablePitch
      }

      uint8_t safe_pitch = getBestAvailablePitch(static_cast<uint8_t>(counter_pitch), current_tick,
  ...
  if (result.empty() && !vocal_analysis.rest_positions.empty()) {
    ...
    for (const Tick& rest_start : vocal_analysis.rest_positions) {
      ...
      uint8_t safe_pitch = getBestAvailablePitch(static_cast<uint8_t>(counter_pitch), rest_start,
      ...
      if (overlap > kSuspensionThreshold) {
        ...
        if (!is_chord_tone) {
          ...
          if (first_duration >= kMinNoteDuration && second_duration >= kMinNoteDuration) {
            ...
            // Use NoteFactory for provenance tracking and pitch safety
            NoteFactory factory(harmony);
            auto safe_note = factory.createIfNoDissonance(chord_change, second_duration,
                                                static_cast<uint8_t>(best_pitch),
                                                static_cast<uint8_t>(note.velocity * 0.9f),
                                                TrackRole::Aux, NoteSource::Aux);
            if (safe_note) {
...
std::vector<NoteEvent> AuxTrackGenerator::generateSustainPad(const AuxContext& ctx,
  ...
  while (current_tick < ctx.section_end) {
    ...
    if (voice_count >= 2 &&
      ...
      uint8_t safe_third =
          getBestAvailablePitch(third_pitch, current_tick, actual_duration, ctx.main_melody, harmony,
                       aux_low, aux_high, current_chord_degree, meta.dissonance_tolerance);
