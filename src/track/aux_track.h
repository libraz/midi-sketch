...
class AuxTrackGenerator {
 public:
  ...
  void generateFullTrack(MidiTrack& track, const SongContext& song_ctx, IHarmonyContext& harmony,
                         std::mt19937& rng);

  /// Generate aux track based on config (single section).
  MidiTrack generate(const AuxConfig& config, const AuxContext& ctx, const IHarmonyContext& harmony,
                     std::mt19937& rng);
