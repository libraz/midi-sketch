...
struct MotifRiffCache {
  ...
  std::vector<NoteEvent> original_pattern;
  bool cached = false;
};
...
std::vector<int> generatePitchSequence(uint8_t note_count, MotifMotion motion, std::mt19937& rng,
  ...
  if (motion == MotifMotion::Ostinato) {
    ...
    static constexpr int kOstinatoVariants[] = {4, 2, 4};  // 5th, 3rd, 5th
    int variant_cursor = rng_util::rollRange(rng, 0, 2);
    for (uint8_t idx = 0; idx < note_count; ++idx) {
      if (idx % 2 == 0) {
        degrees.push_back(0);  // Root anchor on strong subdivisions
      } else {
        degrees.push_back(kOstinatoVariants[variant_cursor % 3]);
        ++variant_cursor;
      }
    }
    return degrees;
  ...
  auto constrainedStep = [max_leap_degrees, prefer_stepwise](int step) {
    // `prefer_stepwise` is a melodic contract, not merely a weaker leap
    // ceiling: retain only adjacent scale-degree motion.  The previous
    // two-degree allowance still produced frequent third/fourth jumps once
    // rendered to semitones, especially across the antecedent/consequent
    // boundary of lyrical (Ballad) motifs.
    int limit = prefer_stepwise ? std::min(1, max_leap_degrees) : max_leap_degrees;
  ...
  for (size_t i = 0; i < positions.size(); ++i) {
    ...
    if (i + 1 < positions.size()) {
      ...
      if (use_template && motif_params.rhythm_template == MotifRhythmTemplate::ChordPulseStabs) {
        note_duration = std::min(gap, static_cast<Tick>(TICK_SIXTEENTH));
...
MotifPitchResult calculateMotifPitch(const NoteEvent& note, const MotifNoteContext& ctx,
  ...
  if (vocal_ctx && motif_params.contrary_motion) {
    ...
    adjusted_pitch = motif_detail::applyContraryMotion(adjusted_pitch, vocal_dir,
                                                       motif_params.contrary_motion_strength, rng);
  }
...
uint8_t calculateMotifVelocity(uint8_t base_vel, bool is_chorus, SectionType section_type,
  ...
  // Fixed velocity means the motif supplies a stable pulse while the drums
  // carry the groove. Do not reintroduce phrase or section dynamics here;
  // the post-processing pipeline intentionally excludes this track as well.
  if (velocity_fixed) {
  ...
  // Phrase-shaped micro-variation for expressive motifs.
  if (is_phrase_start) {
...
uint32_t motifVariationHash(uint32_t seed, size_t section_idx, size_t cycle_idx, size_t onset_idx) {
...
/// LockedPitch/LockedAll patterns are returned verbatim to preserve their
/// fixed-pitch/fixed-riff contracts; LockedContour may still receive light
/// rhythmic expression while preserving the contour.
...
std::vector<NoteEvent> applyCycleRhythmVariation(const std::vector<NoteEvent>& base, uint32_t seed,
  ...
  if (policy == RiffPolicy::LockedPitch || policy == RiffPolicy::LockedAll) return pattern;
...
/// persistent rhythm mutations plus a small pitch mutation so the riff keeps
/// its identity while its onset cell and pitch contour drift across the song.
/// Onset count is kept within [3, 12] per cycle.
void evolveRiffPattern(std::vector<NoteEvent>& pattern, Tick cycle_length, std::mt19937& rng) {
  ...
  static constexpr int kPitchOffsets[] = {2, -2, 5, -5, 7, -7};
  static constexpr size_t kPitchOffsetCount = sizeof(kPitchOffsets) / sizeof(kPitchOffsets[0]);
  size_t pitch_idx =
      static_cast<size_t>(rng_util::rollRange(rng, 1, static_cast<int>(pattern.size()) - 1));
  int offset_cursor = rng_util::rollRange(rng, 0, static_cast<int>(kPitchOffsetCount) - 1);
  int original_pitch = pattern[pitch_idx].note;
  for (size_t attempt = 0; attempt < kPitchOffsetCount; ++attempt) {
    int offset = kPitchOffsets[(offset_cursor + static_cast<int>(attempt)) %
                               static_cast<int>(kPitchOffsetCount)];
    int shifted = snapToNearestScaleTone(original_pitch + offset, 0);
    shifted = std::clamp(shifted, 48, 96);
    if (shifted != original_pitch) {
      pattern[pitch_idx].note = static_cast<uint8_t>(shifted);
      break;
    }
  }
}
...
struct LockedNoteCache {
  Tick source_length = 0;
...
struct MotifGenerationState {
  ...
  std::map<SectionType, LockedNoteCache> locked_note_cache;
...
uint8_t computeVocalCeilingForNote(uint8_t base_range_high, bool enforce_vocal_ceiling,
  ...
  // Keep a small playable search window. If a local vocal dip sits at or below
  // the motif floor, clamping the ceiling to range_low collapses every
  // collision/monotony alternative to one pitch and creates stuck-note runs.
  constexpr uint8_t kMinMotifCeilingSpan = 5;
  uint8_t minimum_ceiling = static_cast<uint8_t>(
      std::min(127, static_cast<int>(range_low) + static_cast<int>(kMinMotifCeilingSpan)));
  if (ceiling < minimum_ceiling) {
    ceiling = minimum_ceiling;
  }
...
bool replayCachedNotesCoordinateAxis(MidiTrack& track, const Section& section,
                                     IHarmonyCoordinator* harmony, MotifGenerationState& state,
                                     uint8_t motif_range_high, uint8_t motif_range_low) {
...
bool isFinalMotifChorus(const std::vector<Section>& sections, size_t section_idx) {
  if (section_idx >= sections.size()) return false;
  const Section& section = sections[section_idx];
  if (section.type != SectionType::Chorus || !hasTrack(section.track_mask, TrackMask::Motif)) {
    return false;
  }
  for (size_t idx = section_idx + 1; idx < sections.size(); ++idx) {
    if (sections[idx].type == SectionType::Chorus &&
        hasTrack(sections[idx].track_mask, TrackMask::Motif)) {
      return false;
    }
  }
  return true;
...
/// @brief Resolve which pattern to use based on RiffPolicy and section state.
/// @param[out] section_pattern Storage for newly generated section pattern (must outlive return)
/// @param[out] hint_pattern Storage for hint-generated pattern (must outlive return)
/// @return Pointer to the pattern to use for this section
std::vector<NoteEvent>* resolveCurrentPattern(
    std::vector<NoteEvent>& pattern, const Section& section, const GeneratorParams& params,
    const MotifParams& motif_params, MotifGenerationState& state, bool is_locked, RiffPolicy policy,
    bool reprise_original_evolving_riff, const MotifRoleMeta& role_meta, std::mt19937& rng,
    std::vector<NoteEvent>& section_pattern, std::vector<NoteEvent>& hint_pattern) {
  // motif_motion_hint override: generate section-specific pattern with hinted motion
  std::vector<NoteEvent>* hint_pattern_ptr = nullptr;
  if (section.motif_motion_hint > 0) {
    GeneratorParams hint_params = params;
    hint_params.motif.motion = static_cast<MotifMotion>(section.motif_motion_hint - 1);
    if (policy == RiffPolicy::Evolving) {
      auto iter = state.hint_patterns.find(section.type);
  ...
  if (policy == RiffPolicy::Evolving && reprise_original_evolving_riff &&
    ...
    if (motif_params.repeat_scope == MotifRepeatScope::FullSong) {
      // Free takes precedence over the otherwise song-wide repeat scope: it
      // must supply a fresh motif identity for every section rather than
      // silently falling through to the original global pattern.
      section_pattern = generateMotifPattern(params, rng);
      current_pattern = &section_pattern;
    } else {
...
/// @brief Determine whether a motif note should be skipped due to variation,
/// phrase tail rest, density thinning, or vocal response mode.
/// @return true if the note should be skipped
bool shouldSkipMotifNote(const Section& section, Tick absolute_tick, Tick pos, size_t cycle_idx,
                         size_t onset_idx, bool is_rhythm_lock_global, uint8_t current_bar,
                         std::map<uint8_t, size_t>& bar_note_count, const GeneratorParams& params,
                         const MotifParams& motif_params, const MotifContext* vocal_ctx,
                         MotifGenerationState& state, std::mt19937& rng) {
  ...
  effective_density =
      static_cast<uint8_t>(std::min(100.0f, static_cast<float>(effective_density) * density_mult));
  ...
  if (!is_rhythm_lock_global && effective_density < 100) {
    bool should_skip = (rng_util::rollFloat(rng, 0.0f, 100.0f) > effective_density);
  ...
  (void)pos;  // Used by caller for bar calculation
  return false;
}

/// @brief Re-apply avoid note correction after vocal ceiling clamp in coordinate axis mode.
/// Clamping may have changed the pitch to an avoid note for the current chord.
...
/// @brief Calculate velocity for a motif note including template mode and micro-variation.
/// @return Final velocity value
uint8_t calculateMotifNoteVelocity(const NoteEvent& note, bool is_chorus,
                                   bool is_rhythm_lock_global, const MotifParams& motif_params,
                                   const MotifRoleMeta& role_meta, SectionType section_type,
                                   size_t cycle_idx, size_t onset_idx, Tick absolute_tick,
                                   uint32_t seed, size_t sec_idx, size_t pattern_size) {
  uint8_t vel;
  if (is_rhythm_lock_global && motif_params.rhythm_template != MotifRhythmTemplate::None) {
    ...
    bool phrase_end = (onset_idx + 1 == pattern_size);
    vel = calculateMotifVelocity(role_meta.velocity_base, is_chorus, section_type,
  ...
  if (cycle_idx > 0) {
    Tick pos_in_bar = absolute_tick % TICKS_PER_BAR;
    if (pos_in_bar > 0) {
      uint32_t var_hash = motifVariationHash(seed, sec_idx, cycle_idx, onset_idx);
...
/// @brief Resolve final pitch applying monotony tracking or coordinate axis safety valve.
/// @return Final pitch value
uint8_t resolveMotifFinalPitch(int adjusted_pitch, bool is_rhythm_lock_global,
                               IHarmonyCoordinator* harmony, Tick absolute_tick,
                               MotifGenerationState& state, uint8_t motif_range_low,
  ...
  if (is_rhythm_lock_global) {
    ...
    uint8_t final_pitch = static_cast<uint8_t>(std::clamp(
        adjusted_pitch, static_cast<int>(motif_range_low), static_cast<int>(motif_range_high)));
    ...
    if (state.motif_consecutive_same > kCoordAxisMonotonyThreshold) {
      final_pitch = selectBestAlternative(final_pitch, harmony, absolute_tick, motif_range_low,
                                          motif_range_high);
    }
...
/// Records provenance transforms and handles octave doubling.
void emitMotifNoteCoordAxis(MidiTrack& track, IHarmonyCoordinator& harmony, const NoteEvent& note,
  ...
  if (added_note_opt) {
    ...
    if (pitch_result.range_octave_up != 0) {
      uint8_t pre_range =
          static_cast<uint8_t>(std::clamp(note.note + pitch_result.section_octave_shift, 0, 127));
      added_note.addTransformStep(TransformStepType::OctaveAdjust, pre_range,
...
/// @brief Emit a motif note in standard mode with collision avoidance.
/// Updates monotony state and handles octave doubling.
/// @return true if note was emitted, false if skipped
bool emitMotifNoteStandard(MidiTrack& track, IHarmonyCoordinator& harmony, const NoteEvent& note,
                           Tick absolute_tick, uint8_t final_pitch, uint8_t vel, bool add_octave,
                           uint8_t motif_range_low, uint8_t motif_range_high,
...
/// @brief Cache section notes for Locked mode replay (both standard and coordinate axis).
void cacheMotifNotes(const MidiTrack& track, const Section& section, bool is_locked,
                     bool is_rhythm_lock_global, MotifGenerationState& state) {
  ...
  if (is_locked && !is_rhythm_lock_global &&
    ...
    for (const auto& evt : track.notes()) {
      if (evt.start_tick >= section.start_tick && evt.start_tick < section.endTick()) {
        entries.push_back(
            {evt.start_tick - section.start_tick, evt.duration, evt.note, evt.velocity});
      }
    }
    if (!entries.empty()) {
      state.locked_note_cache[section.type] = {section.endTick() - section.start_tick,
                                               std::move(entries)};
    }
  }
  ...
  if (is_rhythm_lock_global &&
    ...
    if (!entries.empty()) {
      state.coord_axis_note_cache[section.type] = {section.endTick() - section.start_tick,
                                                   std::move(entries)};
    }
  }
}
...
void generateMotifForSection(MidiTrack& track, const Section& section, const FullTrackContext& ctx,
                             const GeneratorParams& params, MotifGenerationState& state,
                             std::vector<NoteEvent>& pattern, bool is_locked,
                             bool is_rhythm_lock_global, RiffPolicy policy,
                             bool reprise_original_evolving_riff, uint8_t base_note_override,
                             uint8_t motif_range_high, uint8_t motif_range_low, MotifRole role,
  ...
  // Resolve pattern and hint pattern
  ...
  for (Tick pos = section.start_tick; pos < section_end; pos += motif_length, ++cycle_idx) {
    ...
    if (!is_rhythm_lock_global && !reprise_original_evolving_riff &&
        section.max_moving_voices == 0) {
    ...
    for (const auto& note : *cycle_pattern) {
      ...
      // A partial final cycle is valid for short sections. Keep its onsets,
      // but never let a note (or its octave/stab layer) bleed into the next
      // section.
      NoteEvent section_note = note;
      section_note.duration = std::min(note.duration, section_end - absolute_tick);
      if (section_note.duration == 0) {
        ++onset_idx;
        continue;
      }
      ...
      if (shouldSkipMotifNote(section, absolute_tick, pos, cycle_idx, onset_idx,
                              is_rhythm_lock_global, current_bar, bar_note_count, params,
                              motif_params, vocal_ctx, state, rng)) {
        ++onset_idx;
        continue;
      }

      // Apply density_percent for note context
      uint8_t effective_density = section.getModifiedDensity(section.density_percent);
      float density_mult = 1.0f;
      switch (section.getEffectiveBackingDensity()) {
        case BackingDensity::Thin:
      ...
      uint8_t eff_range_high =
          computeVocalCeilingForNote(motif_range_high, enforce_vocal_ceiling, harmony,
                                     absolute_tick, section_note.duration, motif_range_low);
      ...
      MotifPitchResult pitch_result =
          calculateMotifPitch(section_note, note_ctx, params, motif_params, harmony, vocal_ctx,
      ...
      if (is_rhythm_lock_global) {
        ...
        emitMotifNoteCoordAxis(track, *harmony, section_note, absolute_tick, final_pitch, vel,
                               pitch_result, add_octave, add_stab_voice, motif_range_low,
                               eff_range_high);
        bar_note_count[current_bar]++;
      } else {
        if (!emitMotifNoteStandard(track, *harmony, section_note, absolute_tick, final_pitch, vel,
  ...
  // Cache notes for locked/coordinate axis replay
  cacheMotifNotes(track, section, is_locked, is_rhythm_lock_global, state);
}
...
void MotifGenerator::doGenerateFullTrack(MidiTrack& track, const FullTrackContext& ctx) {
  ...
  uint8_t motif_range_low = MOTIF_LOW;
  {
    ...
    if (vocal_high > 0 && vocal_low > 0) {
      ...
      motif_range_high =
          static_cast<uint8_t>(std::min(static_cast<int>(MOTIF_HIGH), vocal_median + 3));
      // Guard against Chord/Bass interference in low register
      motif_range_low = static_cast<uint8_t>(std::max(55, vocal_median - 15));
    } else if (vocal_ctx) {
      // Fallback: use config-based vocal range from MotifContext
      // (for RhythmSync where motif is generated before vocal)
      int vocal_median =
          (static_cast<int>(vocal_ctx->vocal_low) + static_cast<int>(vocal_ctx->vocal_high)) / 2;
      motif_range_high =
          static_cast<uint8_t>(std::min(static_cast<int>(MOTIF_HIGH), vocal_median + 3));
      motif_range_low = static_cast<uint8_t>(std::max(55, vocal_median - 15));
    }
  ...
  state.riff_cache.original_pattern = pattern;
  ...
  for (size_t section_idx = 0; section_idx < sections.size(); ++section_idx) {
    ...
    if (is_rhythm_lock_global) {
      if (replayCachedNotesCoordinateAxis(track, section, harmony, state, motif_range_high,
                                          motif_range_low)) {
        state.sec_idx++;
        continue;
      }
    ...
    bool reprise_original_evolving_riff =
        policy == RiffPolicy::Evolving && isFinalMotifChorus(sections, section_idx);
    generateMotifForSection(track, section, ctx, params, state, pattern, is_locked,
                            is_rhythm_lock_global, policy, reprise_original_evolving_riff,
                            base_note_override, motif_range_high, motif_range_low, role, role_meta,
                            enforce_vocal_ceiling);
